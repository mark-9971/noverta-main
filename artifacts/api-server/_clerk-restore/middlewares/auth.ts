/**
 * CLERK AUTH MIDDLEWARE (restore version)
 *
 * To restore Clerk auth:
 *   cp artifacts/api-server/_clerk-restore/middlewares/auth.ts \
 *      artifacts/api-server/src/middlewares/auth.ts
 *
 * Also restore:
 *   - _clerk-restore/lib/auditLog.ts  → src/lib/auditLog.ts
 *   - _clerk-restore/app.ts           → src/app.ts
 *   - _clerk-restore/routes/supervision-staff-id.ts  (see note inside)
 *   - _clerk-restore/routes/para-staff-id.ts          (see note inside)
 *   - Remove src/routes/auth.ts and its registration in src/routes/index.ts
 */
