...
import { eq, and, isNull, sql, inArray, not, count } from "drizzle-orm";
...
router.get("/data-health", requireAdmin, async (req, res) => {
  try {
    ...
    const studentDistrictFilter = schoolIds.length > 0
      ? sql`(${studentsTable.schoolId} IN (${sql.join(schoolIds.map(id => sql`${id}`), sql`, `)}) OR ${studentsTable.schoolId} IS NULL)`
      : isNull(studentsTable.schoolId);

    const staffDistrictFilter = schoolIds.length > 0
      ? sql`(${staffTable.schoolId} IN (${sql.join(schoolIds.map(id => sql`${id}`), sql`, `)}) OR ${staffTable.schoolId} IS NULL)`
      : isNull(staffTable.schoolId);
    ...
    // 1. Students with no service requirements
    ...
    // 2. Students with no case manager
    ...
    // 3. Students with no school assignment
    ...
    // 4. Service requirements with no assigned provider
    ...
    checks.push({
      ...
      items: reqsNoProvider.slice(0, 25).map(r => ({
        id: r.id,
        label: studentMap.get(r.studentId) || `Student #${r.studentId}`,
        detail: `${serviceTypeMap.get(r.serviceTypeId) || "Unknown service"} · ${r.requiredMinutes} min · No provider`,
      })),
    });

    // 5. Service requirements with zero or missing minute targets
    ...
    // 6. Providers (clinical roles) with no student assignments or schedule blocks
    ...
    // 7. Staff with no email (can't log in)
    ...
    // 8. Duplicate students (same first + last name)
    ...
    const duplicateStudentCount = duplicateGroups.reduce((sum, [, g]) => sum + g.length, 0);
    checks.push({
      ...
      items: duplicateGroups.slice(0, 15).map(([, group]) => ({
        id: group[0].id,
        label: `${group[0].firstName} ${group[0].lastName}`,
        detail: `${group.length} records · IDs: ${group.map(s => s.id).join(", ")}`,
      })),
    });

    // 9. Duplicate staff (same first + last name)
    ...
    // 10. Students with service requirements but no schedule blocks
    ...
    // 12. Students missing key demographics
    ...
    checks.push({
      ...
      category: "data_quality",
      severity: studentsMissingDemo.length > 0 ? "info" : "info",
