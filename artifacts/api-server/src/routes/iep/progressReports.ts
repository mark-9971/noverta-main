...
router.post("/students/:studentId/progress-reports/generate", async (req, res): Promise<void> => {
  try {
    ...
    const goalProgressEntries = await computeGoalProgressEntries(studentId, student.firstName, periodStart, periodEnd);
...
router.post("/progress-reports/admin/backfill-goal-progress", requireRoles("admin"), async (req, res): Promise<void> => {
  try {
    ...
    const allReports = districtId
      ? await baseQuery.where(eq(schoolsTable.districtId, districtId))
      : await baseQuery;
