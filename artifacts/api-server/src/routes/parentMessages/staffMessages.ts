    ...
    if (guardianRow?.email) {
      try {
        const result = await sendEmail({
          ...
          bodyHtml: `<p>You have a new message in the Noverta Parent Portal.</p><p><strong>Subject:</strong> ${subject}</p><p>${body.replace(/\n/g, "<br>")}</p><p>Log in to the portal to reply.</p>`,
          bodyText: `You have a new message: ${subject}\n\n${body}\n\nLog in to the Noverta Parent Portal to reply.`,
