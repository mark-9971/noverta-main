...
import { resolveExportScope, buildCSV, fmtDate, type ReportFilters, ROLE_LABELS } from "./utils";
import {
  ...
  try {
    if (reportType === "compliance-summary") {
      ...
      if (students.length === 0) {
        return { csv: buildCSV(["Student", "School", "Grade", "Service", "Required", "Delivered", "Compliance %", "Status"], []), rowCount: 0 };
      }
