    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-y-auto p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-3xl shadow-xl my-auto max-h-[92vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-slate-100 p-4 md:p-5 flex items-center justify-between z-10 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-slate-800">{isEditing ? "Edit Program" : "Program Builder"}</h2>
        ...
        <div className="flex items-center gap-0 px-4 pt-3 flex-shrink-0">
          {wizardSteps.map((ws, i) => (
            <div key={ws} className="flex items-center">
              ...
              {i < wizardSteps.length - 1 && <ChevronRight className="w-3.5 h-3.5 text-slate-300 mx-1" />}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-5">
          {step === 0 && (
            <div className="space-y-3">
              <p className="text-sm font-medium text-slate-600 mb-3">Select Program Type</p>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="border-t border-slate-100 pt-4">
                <p className="text-[12px] font-semibold text-slate-600 mb-3">Mastery & Regression Criteria</p>
              ...
              <div className="border-t border-slate-100 pt-4">
                <p className="text-[12px] font-semibold text-slate-600 mb-3">Prompt Hierarchy</p>
                    ...
                    return (
                      <div key={level} className={`flex items-center gap-2 p-2 rounded-lg border ${form.currentPromptLevel === level ? "border-indigo-300 bg-indigo-50" : "border-slate-100"}`}>
                        <div className="flex flex-col gap-0.5">
                          ...
                          <button onClick={() => moveStep(idx, "down")} disabled={idx === form.promptHierarchy.length - 1} className="text-slate-400 hover:text-slate-600 disabled:opacity-30">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-[12px] text-slate-600">Auto-progress through prompt hierarchy on mastery</span>
              </label>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-slate-600">Program Steps ({steps.length})</p>
              ...
              <div className="space-y-2">
                {steps.map((s, idx) => (
                  <div key={s.id} className="border border-slate-200 rounded-lg overflow-hidden bg-white">
                    <div className="flex items-center gap-2 p-2.5 cursor-pointer hover:bg-slate-50" onClick={() => setExpandedStep(expandedStep === s.id ? null : s.id)}>
                      <GripVertical className="w-4 h-4 text-slate-300 flex-shrink-0" />
                      <span className="text-[12px] font-bold text-slate-400 w-6">{idx + 1}</span>
                      <input value={s.name} onChange={e => { e.stopPropagation(); updateStep(s.id, "name", e.target.value); }}
                        ...
                        className="flex-1 text-[13px] font-medium text-slate-700 bg-transparent border-none focus:outline-none focus:ring-0" />
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button onClick={e => { e.stopPropagation(); moveStep(idx, "up"); }} disabled={idx === 0}
                          className="p-1 text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowUp className="w-3.5 h-3.5" /></button>
                        <button onClick={e => { e.stopPropagation(); moveStep(idx, "down"); }} disabled={idx === steps.length - 1}
                          className="p-1 text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowDown className="w-3.5 h-3.5" /></button>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <p className="text-sm font-medium text-slate-600">Review Program</p>
              <Card>
                <CardContent className="p-4 space-y-3">
                  ...
                  {form.description && <p className="text-[12px] text-slate-500">{form.description}</p>}
                  {form.domain && <p className="text-[11px] text-slate-400">Domain: {form.domain}</p>}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-2">
                    <div className="bg-slate-50 rounded-lg p-2 text-center">
                      <p className="text-[10px] text-slate-400">Start Prompt</p>
                      <p className="text-[12px] font-bold text-slate-600">{PROMPT_LABELS[form.currentPromptLevel]?.label ?? form.currentPromptLevel}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-2 text-center">
                      <p className="text-[10px] text-slate-400">Mastery</p>
                      <p className="text-[12px] font-bold text-slate-600">{form.masteryCriterionPercent}% x {form.masteryCriterionSessions}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-2 text-center">
                      <p className="text-[10px] text-slate-400">Reinforcement</p>
                      <p className="text-[12px] font-bold text-slate-600 capitalize">{form.reinforcementSchedule.replace(/_/g, " ")}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-2 text-center">
                      <p className="text-[10px] text-slate-400">Auto-Progress</p>
                      <p className={`text-[12px] font-bold ${form.autoProgressEnabled ? "text-emerald-600" : "text-slate-400"}`}>
                  ...
                  {steps.length > 0 && (
                    <div className="pt-2">
                      <p className="text-[11px] font-semibold text-slate-500 mb-1.5">{steps.length} Steps</p>
                      <div className="space-y-1">
                        {steps.map((s, i) => (
                          <div key={s.id} className="flex items-center gap-2 p-1.5 bg-slate-50 rounded text-[12px]">
                            <span className="text-slate-400 font-bold w-5 text-center">{i + 1}</span>
                            <span className="text-slate-700">{s.name || `Step ${i + 1}`}</span>
                            {s.sdInstruction && <span className="text-slate-400 text-[10px]">SD: "{s.sdInstruction}"</span>}
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-slate-100 p-4 flex items-center justify-between flex-shrink-0">
