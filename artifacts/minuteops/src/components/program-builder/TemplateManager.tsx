...
export default function TemplateManager({ studentId, onCloned, onTemplateUpdated }: TemplateManagerProps) {
  ...
  return (
    <div className="space-y-4">
          ...
          {scopeButtons.map(s => (
            <button key={s.key} onClick={() => setScopeFilter(s.key)}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${
                scopeFilter === s.key ? "bg-white text-slate-700 shadow-sm" : "text-slate-500 hover:text-slate-600"
              }`}>
          ...
          {["all", "academic", "behavior"].map(c => (
            <button key={c} onClick={() => setCategoryFilter(c)}
              className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all capitalize ${
                categoryFilter === c ? "bg-white text-slate-700 shadow-sm" : "text-slate-500 hover:text-slate-600"
              }`}>{c}</button>
          ))}
        </div>
        ...
        <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5">
          {([["all", "All"], ["free", "Free"], ["premium", "Premium"]] as const).map(([k, l]) => (
            <button key={k} onClick={() => setTierFilter(k)}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${
                tierFilter === k ? "bg-white text-slate-700 shadow-sm" : "text-slate-500 hover:text-slate-600"
              }`}>
      ...
      {loading ? (
        <div className="py-12 text-center text-slate-400 text-sm">Loading templates...</div>
      ) : filtered.length === 0 ? (
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {t.isGlobal ? (
                      ...
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-medium">Custom</span>
                    )}
                  </div>
                </div>
      ...
      <div className="bg-white rounded-xl w-full max-w-2xl shadow-xl my-auto max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 md:p-5 space-y-4">
          ...
          {template.description && (
            <p className="text-[13px] text-slate-600">{template.description}</p>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div className="bg-slate-50 rounded-lg p-2.5 text-center">
              ...
              <p className="text-sm font-bold text-slate-600">{template.defaultMasteryPercent}% x{template.defaultMasterySessions}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-2.5 text-center">
              <p className="text-[10px] text-slate-400">Regression</p>
              <p className="text-sm font-bold text-slate-600">&lt;{template.defaultRegressionThreshold}%</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-2.5 text-center">
              <p className="text-[10px] text-slate-400">Reinforcement</p>
              <p className="text-sm font-bold text-slate-600 capitalize">{(template.defaultReinforcementSchedule || "CRF").replace(/_/g, " ")}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-2.5 text-center">
          ...
          {template.promptHierarchy && (
            <div>
              <p className="text-[12px] font-semibold text-slate-500 mb-2">Prompt Hierarchy</p>
              <div className="flex flex-wrap gap-1">
                {(template.promptHierarchy as string[]).map((level, i) => (
                  <span key={level} className={`text-[10px] font-medium px-2 py-0.5 rounded ${PROMPT_LABELS[level]?.color ?? "bg-slate-100"}`}>
          ...
          {stepsArr.length > 0 && (
            <div>
              <button onClick={() => setShowSteps(!showSteps)} className="flex items-center gap-1 text-[12px] font-semibold text-slate-600 mb-2">
              ...
              {showSteps && (
                <div className={`space-y-1 ${isPremium ? "opacity-60 select-none" : ""}`}>
                  {stepsArr.slice(0, isPremium ? 3 : undefined).map((s: any, i: number) => (
                    <div key={i} className="flex items-start gap-2 p-2 bg-slate-50 rounded text-[12px]">
                      <span className="text-slate-400 font-bold w-5 text-center flex-shrink-0">{i + 1}</span>
                      <div className="min-w-0">
                        <p className="text-slate-700 font-medium">{s.name}</p>
                        {s.sdInstruction && <p className="text-[10px] text-slate-400 mt-0.5">SD: "{s.sdInstruction}"</p>}
                        {s.targetResponse && <p className="text-[10px] text-slate-400">R: {s.targetResponse}</p>}
                        {s.materials && <p className="text-[10px] text-slate-400">Materials: {s.materials}</p>}
                      </div>
                    </div>
                  ))}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-y-auto p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-2xl shadow-xl my-auto max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-slate-100 p-4 md:p-5 flex items-center justify-between z-10 flex-shrink-0">
          <h2 className="text-lg font-bold text-slate-800">{isNew ? "Create Template" : "Edit Template"}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
        </div>

        <div className="flex gap-1 px-4 pt-3 bg-white flex-shrink-0">
          <button onClick={() => setTab("config")} className={`px-3 py-1.5 rounded-full text-[12px] font-medium ${tab === "config" ? "bg-slate-800 text-white" : "bg-white text-slate-500 border border-slate-200"}`}>
            Configuration
          </button>
          <button onClick={() => setTab("steps")} className={`px-3 py-1.5 rounded-full text-[12px] font-medium ${tab === "steps" ? "bg-slate-800 text-white" : "bg-white text-slate-500 border border-slate-200"}`}>
          ...
          {tab === "config" && (
            <>
              ...
              <div className="border-t border-slate-100 pt-3">
                <p className="text-[12px] font-semibold text-slate-600 mb-2">Scope</p>
                <div className="flex gap-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={form.isGlobal} onChange={() => setForm(f => ({ ...f, isGlobal: true, schoolId: null }))}
                      className="w-4 h-4" />
                    <span className="text-[12px] text-slate-600"><Globe className="w-3 h-3 inline mr-1" />Global (all schools)</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={!form.isGlobal} onChange={() => setForm(f => ({ ...f, isGlobal: false }))}
                      className="w-4 h-4" />
                    <span className="text-[12px] text-slate-600"><Building2 className="w-3 h-3 inline mr-1" />School / Custom</span>
                  </label>
                </div>
              </div>
                  ...
                  <div key={idx} className="border border-slate-200 rounded-lg overflow-hidden bg-white">
                    <div className="flex items-center gap-2 p-2.5 cursor-pointer hover:bg-slate-50"
                      ...
                      <input value={s.name} onClick={e => e.stopPropagation()}
                        ...
                        className="flex-1 text-[13px] font-medium text-slate-700 bg-transparent border-none focus:outline-none" />
                      <div className="flex items-center gap-1">
                        <button onClick={e => { e.stopPropagation(); moveStepFn(idx, "up"); }} disabled={idx === 0}
                          className="p-0.5 text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowUp className="w-3 h-3" /></button>
                        <button onClick={e => { e.stopPropagation(); moveStepFn(idx, "down"); }} disabled={idx === form.steps.length - 1}
                          className="p-0.5 text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowDown className="w-3 h-3" /></button>
                    ...
                    {expandedStep === idx && (
                      <div className="border-t border-slate-100 p-3 space-y-2 bg-slate-50/50">
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="text-[10px] font-medium text-slate-400">SD Instruction</label>
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-2.5 text-[13px] text-slate-600">
          ...
          <div className="bg-slate-50 rounded-xl p-4 text-center">
            <p className="text-[11px] text-slate-400 mb-1">Starting at</p>
            <p className="text-3xl font-bold text-slate-800">$29<span className="text-sm font-normal text-slate-400">/mo</span></p>
            <p className="text-[11px] text-slate-400 mt-1">per school · billed annually</p>
          </div>
          ...
          <button onClick={onClose} className="w-full text-center text-[12px] text-slate-400 hover:text-slate-600">
            Maybe later
          </button>
        </div>
      </div>
