...
export default function IepSearch() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-5">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight">Global IEP Search</h1>
        <p className="text-xs md:text-sm text-slate-400 mt-1">Search across all students' goals, accommodations, and records</p>
      </div>
      ...
      {results && (
        <div className="space-y-4">
          <p className="text-[12px] text-slate-400">{totalResults} result{totalResults !== 1 ? "s" : ""} found</p>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-semibold text-slate-600">Accommodations ({results.accommodations.length})</h3>
                </div>
                <div className="space-y-2">
                  {results.accommodations.map((a: any) => (
                    <Link key={a.id} href={`/students/${a.studentId}/iep`}>
                      <div className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer group">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[12px] font-medium text-slate-700">{a.studentName}</span>
                            <span className="text-[10px] text-slate-400">Grade {a.grade}</span>
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-emerald-50 text-emerald-600 capitalize">{a.category}</span>
                          </div>
                          <p className="text-[12px] text-slate-500">{a.description}</p>
                        </div>
                        <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-slate-500 mt-1" />
                      </div>
                    </Link>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-semibold text-slate-600">Students ({results.students.length})</h3>
                </div>
                <div className="space-y-2">
          ...
          {totalResults === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">Try different keywords or search type</p>
              </CardContent>
            </Card>
          )}
        </div>
      ...
      {!results && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-sm text-slate-400">Search across all students' IEP data</p>
            <p className="text-xs text-slate-400 mt-1">Find goals, accommodations, or students by keyword</p>
          </CardContent>
        </Card>
      )}
    </div>
