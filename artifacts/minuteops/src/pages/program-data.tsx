...
function DataSessionsTab({ dataSessions, onLogSession }: { dataSessions: DataSession[]; onLogSession: () => void }) {
  ...
  function ExpandedSessionDetail({ detail }: { detail: any }) {
    if (expandLoading) {
      return (
        <div className="px-4 py-6 bg-slate-50/80 border-t border-slate-100">
          <div className="flex items-center gap-2 text-sm text-slate-400"><Clock className="w-4 h-4 animate-spin" /> Loading recorded data...</div>
        </div>
      );
    }
    if (!detail) {
      return (
        <div className="px-4 py-4 bg-slate-50/80 border-t border-slate-100 text-sm text-slate-400">
    ...
    return (
      <div className="px-4 py-4 bg-slate-50/80 border-t border-slate-100 space-y-4">
        <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap">
          ...
          {detail.notes && <span className="flex items-center gap-1 text-slate-600">{detail.notes}</span>}
        </div>

        {!hasData ? (
          <p className="text-sm text-slate-400 italic py-2">No behavior or program data recorded in this session.</p>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {behaviors.length > 0 && (
              <div>
                ...
                <div className="space-y-1.5">
                  {behaviors.map((b: any) => (
                    <div key={b.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-slate-200">
                      <div className="min-w-0">
                        <p className="text-[13px] font-medium text-slate-700 truncate">{b.targetName || `Target ${b.behaviorTargetId}`}</p>
                        <p className="text-[10px] text-slate-400">{b.measurementType || "—"}{b.hourBlock ? ` · ${b.hourBlock}` : ""}</p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {programs.map((p: any) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-slate-200">
                        <div className="min-w-0">
                          <p className="text-[13px] font-medium text-slate-700 truncate">{p.targetName || `Program ${p.programTargetId}`}</p>
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {p.notes && <span className="text-[10px] text-slate-400 max-w-[80px] truncate" title={p.notes}>{p.notes}</span>}
                        </div>
                      </div>
                    );
                  })}
              ...
              <Card key={ds.id} className="overflow-hidden">
                <button className="w-full p-3.5 text-left" onClick={() => toggleExpand(ds.id)}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {expandedId === ds.id ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                  </div>
                </button>
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="text-left px-4 py-2.5 text-[11px] text-slate-400 font-semibold uppercase tracking-wider">Recorded Data</th>
                  </tr>
                </thead>
                    ...
                    <Fragment key={ds.id}>
                      <tr
                        ...
                        <td className="px-2 py-2.5 text-center">
                          {expandedId === ds.id ? <ChevronUp className="w-4 h-4 text-slate-400 mx-auto" /> : <ChevronDown className="w-4 h-4 text-slate-300 mx-auto" />}
                        </td>
