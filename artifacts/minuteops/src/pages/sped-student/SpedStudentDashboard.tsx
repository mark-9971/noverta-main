    ...
    <div className="p-4 md:p-6 space-y-6 max-w-4xl mx-auto">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          <p className="text-xs text-slate-400">Case Manager</p>
          <p className="text-sm font-medium text-slate-600">{student?.caseManagerName || "—"}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="pb-2 flex-row items-center justify-between">
            <CardTitle className="text-sm font-semibold text-slate-600">My Support Services</CardTitle>
            ...
            {services.slice(0, 4).map((svc: any, i: number) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-slate-700 truncate">{svc.serviceTypeName || "Support Service"}</p>
                  <p className="text-[11px] text-slate-400">{svc.requiredMinutes || 0} min/week · {svc.deliveryType || "direct"}</p>
                </div>
              </div>
            ))}
            {services.length === 0 && (
              <p className="text-sm text-slate-400 py-4 text-center">No active services found</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {sessions.slice(0, 4).map((sess: any) => (
              <div key={sess.id} className="flex items-center gap-3 p-2 rounded-lg bg-slate-50">
                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${sess.status === "completed" ? "bg-green-500" : "bg-amber-400"}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-slate-700 truncate">{sess.serviceTypeName}</p>
                  <p className="text-[11px] text-slate-400">{sess.sessionDate} · {sess.durationMinutes}min · {sess.staffFirst} {sess.staffLast}</p>
                </div>
            ...
            {sessions.length === 0 && (
              <p className="text-sm text-slate-400 py-4 text-center">No recent sessions</p>
            )}
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <Card className="max-w-lg mx-auto mt-8">
      <CardHeader>
        ...
        <p className="text-sm text-slate-500">Choose a student on an IEP to view their SPED portal</p>
      </CardHeader>
          ...
          {filtered.map(s => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Grade {s.grade} · {s.programName || "SPED"} · CM: {s.caseManagerFirst} {s.caseManagerLast}</p>
              </div>
            </button>
          ))}
