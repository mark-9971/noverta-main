# Trellis Pilot — Known Limitations & Beta Disclosure
...
Trellis is a working product used by a small number of pilot districts. It is not a completed, enterprise-certified SIS. Some parts of the software are stable and well-tested. Others are in active development and may change, break, or be removed. This document tells you which is which.
...
| **Offline access** | Trellis is a web application and requires internet access. There is no offline mode. |
| **File attachments** | File upload is not yet available for IEP documents. Trellis tracks IEP metadata; actual PDF documents are stored outside the system. |
| **DESE direct submission** | Trellis does not currently submit data directly to DESE. The incident and IEP exports are designed to match DESE report formats for manual submission. |
...
- **Daily backups:** The Trellis database is backed up automatically every 24 hours. Backups are retained for 7 days.
...
- **Your responsibility:** Trellis is not a system of record for official IDEA documentation. Your district's paper IEPs and official DESE submissions remain your primary compliance record. Use Trellis as a tracking and workflow layer on top of your existing records.

---

## What Happens If Trellis Is Down for 24 Hours

1. All compliance tracking reverts to your existing manual process temporarily.
2. Trellis will notify the district admin by email as soon as an outage is detected (target: within 30 minutes).
3. If service is not restored within 4 hours, Trellis will provide an estimated restoration time.
...
By proceeding with the Trellis pilot, the district administrator acknowledges that:

1. They have read this document.
2. They understand that Trellis is a pilot product with known limitations.
3. Their district's official IEP records and DESE submissions remain maintained outside of Trellis.
