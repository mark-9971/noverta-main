...
-   **Authentication:** Clerk authentication with real sign-in/sign-up flows. Users must sign in via Clerk to access the app. Routes protected by `ProtectedRoutes` component using `useAuth()`. Sign-in at `/sign-in`, sign-up at `/sign-up`. Sign-out button in sidebar. In dev mode, role switcher allows demo role override via localStorage.
-   **Role-Based Architecture:** 8 roles: `admin`, `case_manager`, `bcba`, `sped_teacher`, `coordinator`, `provider`, `para`, `sped_student`. In production, role comes from Clerk `publicMetadata.role`. In dev, localStorage override is available via the role switcher (gated behind `import.meta.env.DEV`). Each role maps to an appropriate nav config in AppLayout. Staff roles see the admin/teacher nav; `sped_student` sees the student portal nav.
-   **API Auth:** Backend uses `@clerk/express` with `clerkMiddleware()`. `requireAuth` / `requireRoles` / `requireMinRole` middleware read Clerk JWT from Authorization header and validate role from `sessionClaims.publicMetadata.role`. Clerk proxy at `/api/__clerk` (production only). Frontend uses `authFetch` utility that automatically injects Bearer tokens from Clerk `getToken()`.
-   **Clerk Keys:** `VITE_CLERK_PUBLISHABLE_KEY` (frontend), `CLERK_SECRET_KEY` (backend), `CLERK_PUBLISHABLE_KEY` — all stored as Replit secrets.
...
-   **Clinical Supervision Tracking:** Tracks BCBA supervision of RBTs and paraprofessionals per 603 CMR requirements. Page at `/supervision` with two tabs: Session Log (CRUD, filters by supervisor/supervisee/type, expandable row details, CSV export) and Compliance Dashboard (per-supervisee 5% rule compliance with status badges). Staff detail page includes supervision summary section showing last-30-day direct service minutes, required/delivered supervision, compliance status, and recent sessions. Schema: `supervision_sessions` table with supervisorId, superviseeId, sessionDate, durationMinutes, supervisionType (individual/group/direct_observation), topics, feedbackNotes, status (completed/scheduled/cancelled). API endpoints: `/api/supervision-sessions` (CRUD), `/api/supervision/compliance-summary`, `/api/supervision/staff/:staffId/summary`, `/api/supervision-sessions/export/csv`.
